 ...
 *                                currently queued in the gfp queue it will
 ...
 *
 * gfp_job_arrival(T)	- This is the catch all function when T enters
 *                                the system after either a suspension or at a
 *                                job release. It will queue T (which means it
 *                                is not safe to call gfp_job_arrival(T) if
 *                                T is already queued) and then check whether a
 *                                preemption is necessary. If a preemption is
 *                                necessary it will update the linkage
 *                                accordingly and cause scheduled to be called
 *                                (either with an IPI or need_resched). It is
 *                                safe to call gfp_job_arrival(T) if T's
...
DEFINE_PER_CPU(cpu_entry_t, gfp_cpu_entries);
...
cpu_entry_t* gfp_cpus[NR_CPUS];

/* the cpus queue themselves according to priority in here */
static struct bheap_node gfp_heap_node[NR_CPUS];
...
static void update_cpu_position(cpu_entry_t *entry)
{
	if (likely(bheap_node_in_heap(entry->hn)))
		bheap_delete(cpu_lower_prio, &gfp_cpu_heap, entry->hn);
	bheap_insert(cpu_lower_prio, &gfp_cpu_heap, entry->hn);
}

/* caller must hold gfp lock */
static cpu_entry_t* lowest_prio_cpu(void)
{
	struct bheap_node* hn;
	hn = bheap_peek(cpu_lower_prio, &gfp_cpu_heap);
	return hn->value;
}
				      ...
				      cpu_entry_t *entry)
{
	...
	if (linked) {
		...
		if (on_cpu != NO_CPU) {
			sched = &per_cpu(gfp_cpu_entries, on_cpu);
 ...
 *          where it was linked before. Must hold gfp_lock.
 */
static noinline void unlink(struct task_struct* t)
{
	...
	if (t->rt_param.linked_on != NO_CPU) {
		/* unlink */
		entry = &per_cpu(gfp_cpu_entries, t->rt_param.linked_on);
...
#ifdef CONFIG_SCHED_CPU_AFFINITY
static cpu_entry_t* gfp_get_nearest_available_cpu(cpu_entry_t *start)
{
...
static void check_for_preemptions(void)
{
	 ...
	 * idle. */
	local = this_cpu_ptr(&gfp_cpu_entries);
	task  = fp_prio_peek(&gfp.ready_queue);
	...
	for (last = lowest_prio_cpu();
	     fp_preemption_needed(&gfp.ready_queue, last->linked);
		...
		{
			cpu_entry_t *affinity =
					gfp_get_nearest_available_cpu(
						&per_cpu(gfp_cpu_entries, task_cpu(task)));
			if (affinity)
				last = affinity;
...
static noinline void gfp_job_arrival(struct task_struct* task)
{
	printk("gfp_job_arrival() starts.\n");
	BUG_ON(!task);
	requeue(task);
	check_for_preemptions();
	printk("gfp_job_arrival() ends.\n");
}
...
static struct task_struct* gfp_schedule(struct task_struct * prev)
{
...
#ifdef WANT_ALL_SCHED_EVENTS
	TRACE_TASK(prev, "invoked gfp_schedule.\n");
#endif
	...
	raw_spin_unlock(&gfp_lock);

#ifdef WANT_ALL_SCHED_EVENTS
	TRACE("gfp_lock released, next=0x%p\n", next);
...
static void gfp_task_exit(struct task_struct * t)
{
	...
	if (tsk_rt(t)->scheduled_on != NO_CPU) {
		gfp_cpus[tsk_rt(t)->scheduled_on]->scheduled = NULL;
		tsk_rt(t)->scheduled_on = NO_CPU;
	}
...
static struct domain_proc_info gfp_domain_proc_info;
static long gfp_get_domain_proc_info(struct domain_proc_info **ret)
{
	*ret = &gfp_domain_proc_info;
	return 0;
}

static void gfp_setup_domain_proc(void)
{
	int i, cpu;
	int release_master =
#ifdef CONFIG_RELEASE_MASTER
	...
	memset(&gfp_domain_proc_info, 0, sizeof(gfp_domain_proc_info));
	init_domain_proc_info(&gfp_domain_proc_info, num_rt_cpus, 1);
	gfp_domain_proc_info.num_cpus = num_rt_cpus;
	gfp_domain_proc_info.num_domains = 1;

	gfp_domain_proc_info.domain_to_cpus[0].id = 0;
	for (cpu = 0, i = 0; cpu < num_online_cpus(); ++cpu) {
		if (cpu == release_master)
			continue;
		map = &gfp_domain_proc_info.cpu_to_domains[i];
		map->id = cpu;
		...
		cpumask_set_cpu(cpu,
			gfp_domain_proc_info.domain_to_cpus[0].mask);
	}
}

static long gfp_activate_plugin(void)
{
	int cpu;
	cpu_entry_t *entry;
	...
	gfp_setup_domain_proc();

	return 0;
}

static long gfp_deactivate_plugin(void)
{
	destroy_domain_proc_info(&gfp_domain_proc_info);
	return 0;
}
...
static struct sched_plugin gfp_plugin __cacheline_aligned_in_smp = {
	...
	.task_exit		= gfp_task_exit,
	.schedule		= gfp_schedule,
	.task_wake_up		= gfp_task_wake_up,
	.task_block		= gfp_task_block,
	.admit_task		= gfp_admit_task,
	.activate_plugin	= gfp_activate_plugin,
	.deactivate_plugin	= gfp_deactivate_plugin,
	.get_domain_proc_info	= gfp_get_domain_proc_info,
};


static int __init init_gfp(void)
{
	int cpu;
	cpu_entry_t *entry;
